...
Matrix _sample(int rows, int cols) => Matrix(
  List.generate(
    ...
    (r) => List.generate(cols, (c) {
      ...
      final n = (r == 0 && c == 0) ? 0 : ((r + c).isEven ? 1 : -1) * (r * 3 + c + 2);
...
Map<String, StepSolution> _lessons() {
  ...
  for (var n = 1; n <= 5; n++) {
    ...
    out['multiply $n'] =
        MatrixArithmeticSolver.multiply(_sample(n, n), _sample(n, n));
    if (n >= 2) {
  ...
  out['multiply 2x5*5x3'] =
      MatrixArithmeticSolver.multiply(_sample(2, 5), _sample(5, 3));
...
void main() {
  ...
  group('Every lesson animates to completion', () {
    lessons.forEach((label, solution) {
      testWidgets(label, (tester) async {
        ...
        while (elapsed < cap && !(cubit.state.currentStepIndex == total - 1 &&
        ...
        expect(cubit.state.currentStepIndex, total - 1,
            reason: '$label: stalled at step ${cubit.state.currentStepIndex} '
  ...
  testWidgets('Largest lessons stay exception-free in dark theme', (
    ...
    for (final label in ['rref 5', 'lu 5', 'multiply 5', 'determinant 5']) {
      ...
      await tester.pump();
      final cubit =
          tester.element(find.byType(MatrixDisplayGrid)).read<PlayerCubit>();
      cubit.setPlaybackSpeed(4);
      for (var i = 0; i < 200; i++) {
