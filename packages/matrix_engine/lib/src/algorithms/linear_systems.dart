...
class LinearSystemsSolver {
  ...
  static StepSolution solve(Matrix augmentedMatrix) {
    ...
    if (totalCols < 2) {
      throw ArgumentError(
        'Augmented matrix must have at least 2 columns [A | b].',
      );
    }
    ...
    if (freeCols.isEmpty && pivotCols.length == varCount) {
      ...
      for (int c = 0; c < varCount; c++) {
        ...
        highlights.add(
          CellHighlight(row: r, col: varCount, type: HighlightType.selected),
        );
      ...
      final vectorLatex =
          '\\mathbf{x} = \\begin{pmatrix}$solValStr\\end{pmatrix}';
      ...
      steps.add(
        MatrixStep(
          ...
          explanationParams: {'solution': solStrings.join(', ')},
          matrixBefore: rrefSnap,
          matrixAfter: rrefSnap,
    ...
    for (int i = 0; i < freeCols.length; i++) {
      freeParamMap[freeCols[i]] = i < paramNames.length
          ? paramNames[i]
          : 't_{${i + 1}}';
    }
